...
function getOverlapType(item, other) {
  if (
    ...
    return "inner";
  } else if (other.end > item.start && other.end < item.end) {
    return "left";
  } else if (other.start > item.start && other.start < item.end) {
    return "right";
  }
...
// Given a list of items, returns necessary ops to make items not overlapping by
// cutting and splitting them.
function computeOps(items, otherItems) {
  return items
    .filter(it => !!it)
    .reduce((acc, item) => {
      return acc.concat(
        otherItems
          .filter(it => it.group === item.group && it.id !== item.id)
          .reduce((acc2, it) => {
            switch (getOverlapType(item, it)) {
              case "over":
                return acc2.concat({ type: "remove", item: it });
              case "inner":
                return acc2.concat(
                  {
                    type: "update",
                    item: Object.assign({}, it, { end: item.start })
                  },
                  {
                    type: "add",
                    item: Object.assign({}, it, { id: "NEW", start: item.end })
                  } // TODO: NEW
                );
              case "left":
                return acc2.concat({
              ...
              case "right":
                return acc2.concat({
                  type: "update",
                  item: Object.assign({}, it, { start: item.end })
                });
              default:
                return acc2;
            }
...
ItemSet.prototype.redraw = function() {
  ...
  var changedStackSubgroupsOption =
    ...
    function(id) {
      ...
      if (!item && itemData) {
        ...
        if (constructor) {
          ...
          throw new TypeError(
            'Item type "rangeoverflow" is deprecated. Use css styling instead: ' +
...
ItemSet.prototype._onRemove = function(ids) {
  var count = 0;
  var me = this;
  var before = ids.map(id => me.items[id].data);
  ...
  if (count) {
    ...
    // Here we do not call _onAfterChanges since we can't get items.
    this.body.emitter.emit("changes", { added: [], updated: [], deleted: ids });
  }
};

ItemSet.prototype._onAfterChanges = function(before, after) {
  // First, we compute needed ops after changes (split, cut etc).
  const diffed = Object.keys(diff(before, after))
    .map(it => parseInt(it, 10))
    .map(i => after[i].id);
  const changed = after.filter(it => diffed.includes(it.id));
  const others = after.filter(it => !diffed.includes(it.id));

  // Then we compute what ops are needed to avoid overlapping and execute them.
  computeOps(changed, others).forEach(({ type, item }) => {
    if (type === "remove") {
      this.itemsData.remove(item.id);
    } else {
      this.itemsData[type](item);
    }
  });

  // Finally we can emit changes.
  const afterOps = this.itemsData.get();
  const detailedDiffed = detailedDiff(before, afterOps);

  const changes = {
    added: Object.keys(detailedDiffed.added).map(i =>
      Object.assign({}, detailedDiffed.added[i], {
        id: afterOps[parseInt(i, 10)].id
      })
    ),
    updated: Object.keys(detailedDiffed.updated).map(i =>
      Object.assign({}, detailedDiffed.updated[i], {
        id: afterOps[parseInt(i, 10)].id
      })
    ),
    deleted: Object.keys(detailedDiffed.deleted).map(
      i => before[parseInt(i, 10)].id
    )
  };

  this.body.emitter.emit("changes", changes);
};
...
ItemSet.prototype._onRemoveGroups = function(ids) {
  ...
  var after = this.itemsData.get();

  this._onAfterChanges(before, after);
};
...
ItemSet.prototype._onDragStart = function(event) {
  ...
  if (item && (item.selected || this.options.itemsAlwaysDraggable.item)) {
    ...
    if (dragLeftItem) {
      ...
      this.touchParams.itemProps = itemsToDrag.map(
        function(id) {
          var item = me.items[id];
...
ItemSet.prototype._onDrag = function(event) {
  if (this.touchParams.itemProps) {
    ...
    var updateGroupAllowed =
      ((this.options.editable.overrideItems || selectedItem.editable == null) &&
        this.options.editable.updateGroup) ||
      (!this.options.editable.overrideItems &&
    ...
    this.touchParams.itemProps.forEach(
      function(props) {
          ...
          if (props.dragLeft) {
            ...
            if (this.options.rtl) {
              if (itemData.end != undefined) {
                initialEnd = props.data.end;
                end = initialEnd + offset;
                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
                itemData.end = snap ? snap(end, scale, step) : end;
              }
            } else {
              if (itemData.start != undefined) {
                initialStart = props.data.start;
                start = initialStart + offset;
                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
            ...
            if (this.options.rtl) {
              if (itemData.start != undefined) {
                initialStart = props.data.start;
                start = initialStart.valueOf() + offset;
                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
                itemData.start = snap ? snap(start, scale, step) : start;
              }
            } else {
            ...
            if (itemData.start != undefined) {
              ...
              if (itemData.end != undefined) {
                initialEnd = props.data.end;
                var duration = initialEnd - initialStart;

                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
                itemData.start = snap ? snap(start, scale, step) : start;
                itemData.end = itemData.start + duration;
              } else {
                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
                itemData.start = snap ? snap(start, scale, step) : start;
              }
        ...
        if (
          updateGroupAllowed &&
          (!props.dragLeft && !props.dragRight) &&
          newGroupBase != null
        ) {
        ...
        me.options.onMoving(
          ...
          function(itemData) {
            if (itemData) {
              props.item.setData(this._cloneItemData(itemData));
            }
          }.bind(this)
        );
      }.bind(this)
...
ItemSet.prototype._onGroupClick = function(event) {
  ...
  var nestedGroups = groupsData
    .get(group.nestedGroups)
  ...
  if (nestingGroup.showNested) {
    ...
    var collapsedDirClassName = this.options.rtl
      ? "collapsed-rtl"
      : "collapsed";
      ...
      if (!util.equalArray(newOrder, this.groupTouchParams.originalOrder)) {
        ...
        while (curPos < numGroups) {
          ...
          // group at the position where other one should be, fix index arrays and continue
            ...
            var slippedPosition = newOrder.indexOf(
...
ItemSet.prototype._onMultiSelectItem = function(event) {
  ...
  if (item) {
    ...
    if (shiftKey && this.options.multiselect) {
      ...
      var range = ItemSet._getItemRange(
        this.itemsData.get(selection, this.itemOptions)
      );
